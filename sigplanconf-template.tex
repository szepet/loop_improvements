...
\maketitle
% TODO code examples, infrastructure, explodedgraph example, Eval, reference, 
%acknowledgment
...
\begin{enumerate*} [label={(\arabic*)}, noitemsep]
	\item loops worth to fully unroll were determined with applied heuristics
...
simply containers of statements. The \texttt{CFGBlocks}s essentially represents 
the basic blocks of the code but can contain some extra custom information. So 
for the sake of easier understanding TODO amikor azt mondjuk, hogy basic block, 
akkor CFGBlock-ra gondolunk, de ez segit elkepzelni/ jobban 
kifejezni/szemleletesebbe teszi a peldakat, ha basic blockot kepzelunk magunk 
ele.

So -- based on the function CFGs -- we build up an \texttt{ExplodedGraph}. A 
...
it encounters a modified variable which cannot be handled by the invalidation
...
create nodes containing invalid states.
This approach helps us to cover cases and find bugs like the one 
illustrated on Listing \ref{WidenEx}, and still not report false positives 
which are represented on Listing \ref{WidenProb}.
...
various C/C++ open source project. These are the following:
