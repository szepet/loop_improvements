...
\begin{document}
	...
	the 
	rest of the article we will use the term of basic blocks for 
	\texttt{CFGBlocks} 
	...
	On most of the projects we managed to strictly increase analysis coverage 
	...
	Listing \ref{tab:pathnum} presents the numbers of analyzed execution paths.
	...
	\begin{table}
		...
		\begin{tabular}{ |c||c|c|c|c| } 
			...
			\hline
		\end{tabular}
		\caption{The number of bug reports produced by the 
	...
	%does. 
	%This method fits in the current toolset which makes it implementable very 
	%easy.
	
	%In this article I managed to extend the nodes of the exploded graph by 
	%some 
	%custom information about loop processing and I implemented two methods by 
	%which 
	%a bigger coverage could be reached. Unrolling the not too complex loops 
	%based 
	%on some heuristics made it possible to continue the analysis after the 
	%loops 
	%and widening loops made it possible to determine variable constraints 
	%which 
	%are 
	%closer to reality.
